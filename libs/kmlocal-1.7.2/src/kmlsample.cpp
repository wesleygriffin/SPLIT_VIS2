...
//	printPt(cout, dataPts[i]);
    ...
    //cout << "\nExecuting Clustering Algorithm: Lloyd's"<<endl;;
    if(!predefined)
   	 ctrs = kmLloyds.execute();			// execute
    else
	{kmLloyds.setDefinedCenters(centers);
   	 ctrs = kmLloyds.execute_predefined();}			// execute
...
void getArgs(int argc, char **argv)
{
    ...
    if (argc <= 1) {				// no arguments
  	cerr << "Usage:\n\n"
        << "   kmlsample [-d dim] [-k nctrs] [-max mpts] [-df data] [-s stages] [-p predefined centers]\n"
        << "\n"
        << " where\n"
        << "    dim             Dimension of the space (default = 2)\n"
        << "    nctrs           The number of centers (default = 4)\n"
	...
	else if (!strcmp(argv[i], "-o")) {	// -s option
	    ifstream infile(argv[++i], ios::in);
		centers = kmAllocPts(k, dim);
		for(int i=0;i<k;i++)
		{
			double p;
			for(int j=0;j<dim;j++)
