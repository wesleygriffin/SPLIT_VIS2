...
protected:
    ...
    KMpointArray	defined_centers;	//by Henan
...
public:
	...
	defined_centers = kmAllocPts(kCtrs, dim);
    }

    virtual ~KMlocal() { }			// virtual destructor
    virtual KMfilterCenters execute();		// execute the algorithm
    virtual KMfilterCenters execute_predefined();	// execute the algorithm

    void setDefinedCenters(KMpointArray a) { //by henan
	for(int i=0;i<kCtrs;i++)
	{
		kmCopyPt(dim, a[i], defined_centers[i]);
	}
    }
...
protected:					// overridden by subclasses
    ...
    virtual void reset_predefined() {			// reset everything by henan
	stageNo = 0;
	runInitStage = 0;
	curr.genRandom_predefined(defined_centers);	// pre-defined centers
	curr.getDist();				// compute initial distortion
	best = curr;
    }
...
protected:					// overridden methods
    virtual void reset() {
    	...
    	KMlocal::reset_predefined();			// reset base class
